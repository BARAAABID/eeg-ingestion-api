...
# Function to populate initial data without 'eeg_data' column
...
def fix_existing_records():
    conn = get_db_connection()
    cursor = conn.cursor()
    # Ensure that the existing records have an id if they are missing
    cursor.execute("SELECT rowid, * FROM eeg_records")  # rowid is a SQLite-specific feature
    records = cursor.fetchall()
    
    for record in records:
        if not record['id']:  # If 'id' is None (which shouldn't happen with AUTOINCREMENT)
...
async def create_record(record: EEGRecord):
    ...
    conn.commit()
    new_record_id = cursor.lastrowid  # Automatically generated id
    conn.close()
